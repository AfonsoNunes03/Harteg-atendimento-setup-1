...
> Repositório interno da Harteg.
> Ao final desta sessão vais ter WhatsApp + Email ligados, um agente IA a responder a leads e disparos programados a funcionar — sem digitar um único comando.
...
| Disparos programados | WhatsApp + Email para sua lista de contactos |
| Rate limiter ativo | Proteção anti-bloqueio (30 msgs/hora, 90s entre envios) |
| Monitor a correr | Avisa no seu WhatsApp se algo cair |
...
```bash
git clone [link-do-repo] harteg-atendimento-setup-1
cd harteg-atendimento-setup-1
claude
```

O Claude lê as instruções e conduz-te por tudo — sem precisares de digitar mais nada.
...
| Heartbeat 3 camadas | — | ✅ |
| Copy com IA (geração automática) | — | ✅ Fase 3 |
| CRM com IA | — | ✅ Fase 4 |
...
│   ├── import_contacts.py       # Importa contactos por texto ou CSV
...
└── docs/
    └── prerequisitos.md         # Guia de instalação por sistema operativo
```
...
Abre uma issue neste repositório com a descrição do problema.
